...
export function ChatView({ session, pageContext, welcomeMessage }: Props) {
  const { executeActions } = useBridge();
  const { messages, isStreaming, sendMessage, setActionsHandler } = useChat(
    session.token,
    pageContext
  );
  ...
  // Wire bridge actions handler once
  useEffect(() => {
